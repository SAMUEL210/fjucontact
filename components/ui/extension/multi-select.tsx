...
>(({ className, children, ...props }, ref) => {
  const { value, onValueChange, activeIndex, data } = useMultiSelect();
      ...
      {value.map((item, index) => (
        ...
        >
          <span className="text-xs">{
            `${data.filter((Jeune) => (Jeune.telephone == item))[0].prenom} ${data.filter((jeune) => (jeune.telephone == item))[0].nom}`
          }</span>
          <button
...
>(({ className, children }, ref) => {
  return (
    ...
    >
      {children}
      <CommandEmpty>
        <span className="text-muted-foreground">Pas de résultats</span>
      </CommandEmpty>
    </CommandList>
  );
});
